...
This directory contains files to integrate Horalix P2P in your desktop environment (DE).
Specifically this works for DEs that implement the [XDG Desktop Menu Specification][1], which
is virtually every DE.  
To add Horalix P2P to desktop menus for all users, copy the `.desktop` files to
`/usr/local/share/applications` (root required). To add it for just your user, copy them to `~/.local/share/applications`.  
To start Horalix P2P automatically, you have two options: Either you go to the autostart settings of your DE and choose Horalix P2P or you copy the `syncthing-start.desktop` file to `~/.config/autostart`.  
