# Docker Container for Horalix P2P

Use the Dockerfile in this repo, or pull the `kerim-sabic/horalixp2pvibe` image
from Docker Hub.

Use the `/var/horalix` volume to have the synchronized files available on the
host. You can add more folders and map them as you prefer.

Note that Horalix P2P runs as UID 1000 and GID 1000 by default. These may be
altered with the `PUID` and `PGID` environment variables. In addition
the name of the Horalix P2P instance can be optionally defined by using
`--hostname=horalix` parameter.

To grant Horalix P2P additional capabilities without running as root, use the
...
**Docker cli**
```
$ docker pull kerim-sabic/horalixp2pvibe
$ docker run --network=host  -e STGUIADDRESS= \
    -v /wherever/st-sync:/var/horalix \
    kerim-sabic/horalixp2pvibe:latest
```
...
services:
  syncthing:
    image: kerim-sabic/horalixp2pvibe
    container_name: horalix
    hostname: my-horalix
    environment:
      - PUID=1000
      - PGID=1000
...
from being discovered, as Horalix P2P can only see the internal IP address of
...
Be aware that horalix alone is now in control of what interfaces and ports it
listens on. You can edit the horalix configuration to change the defaults if
there are conflicts.

## GUI Security

By default Horalix P2P inside the Docker image listens on `0.0.0.0:8384`. This
allows GUI connections when running without host network mode. The example
above unsets the `STGUIADDRESS` environment variable to have Horalix P2P fall
