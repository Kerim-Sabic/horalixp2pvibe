angular.module('horalix.core')
    ...
    .controller('SyncthingController', function ($scope, $http, $location, LocaleService, Events, $filter, $q, $compile, $timeout, $rootScope, $translate) {
        ...
        function initController() {
            ...
            if (!$scope.authenticated) {
                function setVersionFromHeader(_data, _status, headers) {
                    var version = headers('X-Horalix-Version');
                    if (version) {
                            ...
                            $scope.restoreVersions.tree = $("#restoreTree").fancytree({
                                ...
                                renderColumns: function (event, data) {
                                    ...
                                    var node = data.node,
                                        ...
                                        template;
                                    if (node.folder) {
                                        template = '<div ng-include="\'horalix/folder/restoreVersionsMassActions.html\'"/>';
                                    } else {
                                        template = '<div ng-include="\'horalix/folder/restoreVersionsVersionSelector.html\'"/>';
                                    }
        ...
        $scope.docsURL = function (path) {
            var url = 'https://docs.horalix.com';
        ...
        $scope.shareDeviceIdDialog = function (method) {
            ...
            var title = $translate.instant('Horalix P2P device ID for "{%devicename%}"', {devicename: deviceName});
            var footer = $translate.instant("Learn more at {%url%}", {url: "https://horalix.com"});
            ...
            switch (method) {
                case "email":
                    ...
                    params.body = [
                        $translate.instant('To connect with the Horalix P2P device named "{%devicename%}", add a new remote device on your end with this ID:', {devicename: deviceName}),
                        deviceID,
                        $translate.instant("Horalix P2P is a continuous file synchronization program. It synchronizes files between two or more computers in real time, safely protected from prying eyes. Your data is your data alone and you deserve to choose where it is stored, whether it is shared with some third party, and how it's transmitted over the internet."),
                        footer
                    ].join('\r\n\r\n');
                    break;
                case "sms":
    ...
    .directive('shareTemplate', function () {
        return {
            templateUrl: 'horalix/core/editShareTemplate.html',
            scope: {
                selected: '=',
                encryptionPasswords: '=',
                id: '@',
