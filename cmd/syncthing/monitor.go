...
func (c *serveCmd) monitorMain() {
	...
	if err != nil {
		slog.Error("Failed to start the main Horalix P2P process", slogutil.Error(err))
		panic("Error starting the main Horalix P2P process")
	}
	...
	for {
		...
		slog.Info("Horalix P2P exited", slogutil.Error(err))
