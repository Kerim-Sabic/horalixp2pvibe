...
file at the path specified by --log-file=path. In addition to a path, the special
values "default" and "-" may be used. The former logs to DATADIR/horalix.log
...
The following environment variables modify Horalix P2P's behavior in ways that
...
type CLI struct {
	...
	Serve serveCmd `cmd:"" help:"Run Horalix P2P (default)" default:"withargs"`
	CLI   cli.CLI  `cmd:"" help:"Command line interface for Horalix P2P"`
...
type serveCmd struct {
	...
	NoRestart                 bool          `help:"Do not restart Horalix P2P when exiting due to API/GUI command, upgrade, or crash" env:"STNORESTART"`
...
func (c *serveCmd) syncthingMain() {
	...
	if err != nil {
		...
		slog.Error("Failed to acquire lock: is another Horalix P2P instance already running?")
		os.Exit(1)
	}
	...
	if err != nil {
		slog.Error("Failed to start Horalix P2P", slogutil.Error(err))
		os.Exit(svcutil.ExitError.AsInt())
	}
	...
	if status == svcutil.ExitError {
		slog.Error("Horalix P2P stopped with error", slogutil.Error(app.Error()))
	}
