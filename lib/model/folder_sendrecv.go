...
var (
	...
	errIncompatibleSymlink    = errors.New("incompatible symlink entry; rescan with newer Horalix P2P on source")
...
func (f *sendReceiveFolder) handleSymlink(file protocol.FileInfo, dbUpdateChan chan<- dbUpdateJob, scanChan chan<- string) {
	...
	if len(file.SymlinkTarget) == 0 {
		// Index entry from a Horalix P2P predating the support for including
