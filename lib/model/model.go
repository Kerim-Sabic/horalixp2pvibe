...
var (
	...
	errStopped = errors.New("Horalix P2P is being stopped") //nolint:staticcheck //nolint:staticcheck
...
func (m *model) warnAboutOverwritingProtectedFiles(cfg config.FolderConfiguration, ignores *ignore.Matcher) {
	...
	if len(filesAtRisk) > 0 {
		slog.Warn("Some protected files may be overwritten and cause issues; see https://horalix.com/docs/users/config#syncing-configuration-files for more information", slog.Any("filesAtRisk", filesAtRisk))
	}
}
...
func (m *model) removeFolder(cfg config.FolderConfiguration) {
	...
	if isPathUnique {
		...
		// otherwise not removable) Horalix P2P-specific marker files.
...
func (m *model) ClusterConfig(conn protocol.Connection, cm *protocol.ClusterConfig) error {
	...
	for _, info := range ccDeviceInfos {
		if deviceCfg.Introducer && info.local.Introducer {
			slog.Error("Remote is an introducer to us, and we are to them - only one should be introducer to the other, see https://horalix.com/docs/users/introducer", deviceCfg.DeviceID.LogAttr())
		}
		break
	}
