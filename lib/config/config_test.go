...
func TestDefaultValues(t *testing.T) {
	...
	expected := Configuration{
		...
		Options: OptionsConfiguration{
			...
			URURL:                     "https://data.horalix.com/newdata",
			URInitialDelayS:           1800,
			URPostInsecurely:          false,
			ReleasesURL:               "https://upgrades.horalix.com/meta.json",
			AlwaysLocalNets:           []string{},
			OverwriteRemoteDevNames:   false,
			TempIndexMinBlocks:        10,
			UnackedNotificationIDs:    []string{"authenticationUserAndPassword"},
			SetLowPriority:            true,
			CRURL:                     "https://crash.horalix.com/newcrash",
			CREnabled:                 true,
...
func TestV14ListenAddressesMigration(t *testing.T) {
	tcs := [][3][]string{
		...
		{
			{"tcp://0.0.0.0:22000"},
			{"dynamic+https://relays.horalix.com/endpoint"},
			{"default"},
		},
		...
		// Non-default listen plus default relays gets copied verbatim
		{
			{"tcp://1.2.3.4:22000"},
			{"dynamic+https://relays.horalix.com/endpoint"},
			{"tcp://1.2.3.4:22000", "dynamic+https://relays.horalix.com/endpoint"},
		},
		// Default stuff gets sucked into "default", the rest gets copied
		{
			{"tcp://0.0.0.0:22000", "tcp://1.2.3.4:22000"},
			{"dynamic+https://relays.horalix.com/endpoint", "relay://other.example.com"},
